...
input_image = np.expand_dims(imread('./images.jpg'), axis=0).astype('float32')

net,_ = vgg_net(input_image)
...
for keys in net:
  ...
  tf.summary.image(str(keys) + '_features', output_show, 10)

merge_summary_op = tf.summary.merge_all()
summary_writer = tf.summary.FileWriter('./visual_logs')

with tf.Session() as sess:
  saver.restore(sess,'./progress/')
